...
class SAR(nn.Module):
    ...
    """
    def __init__(self, model, lr, episodic=False, reset_constant=0.2, steps=1, e_margin=0.4*math.log(10000)):
        super().__init__()
        self.model, self.optimizer = self.prepare_SAR_model_and_optimizer(model,lr)
        self.steps = steps
    ...
    @staticmethod
    def prepare_SAR_model_and_optimizer(model, lr):
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    return output_test, ema, reset_flag
